...
const App = () => {
  ...
  const navBar = () => (
    <Navbar color='dark' dark>
      <Container>
        <NavbarBrand href='/'>
          <img className='d-inline-block align-top' id='lock' src={lock} alt='' /> Passer
        </NavbarBrand>
      </Container>
    </Navbar>
  );
    ...
    <Router>
      ...
      <Container className='app-container' role='main'>
        <Routes>
          ...
          />
          <Route path={config.Path.HOW_IT_WORKS} element={<HowItWorks />} />
          <Route path={config.Path.ENCRYPT} element={<Encrypt setAlerts={setAlerts} />} />
        </Routes>
      </Container>
    </Router>
  );
  ...
  const footer = () => (
    <Footer>
      Copyright © {new Date().getFullYear()} Marcelo Lima | Icons provided by{' '}
      <a href='https://fontawesome.com/license'>Font Awesome</a> | Source code available on{' '}
      <a href='https://github.com/m-lima/passer'>GitHub</a>
    </Footer>
  );
