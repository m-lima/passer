...
const Encrypt = (props: IProps) => {
  ...
  const inputModal = () => (
    ...
    >
      ...
      <ModalFooter>
        <Button color='success' disabled={secretText.length === 0} onClick={encryptText}>
          Encrypt
        </Button>
  ...
  const packItem = (pack: pack.Encrypted, key: number) => (
    <ListGroupItem key={key}>
      <div className='spread'>
        <Glyph src={lock}>{`${pack.hash} (${pack.name})`}</Glyph>
        <span>
  ...
  const packList = () => (
    <>
      <div className='enc-packs'>
        <ListGroup flush>{packs.map(packItem)}</ListGroup>
      </div>
      {Number.parseInt(sizePercentage) >= 10 ? (
        <Progress color='info' value={sizePercentage} className='enc-progress'>
          <span className='enc-progresss-value'>
            {sizePercentage}
            {' %'}
          </span>
        </Progress>
      ...
      <Button color='success' size='lg' block onClick={send} disabled={totalSize > pack.MAX_SIZE}>
        Upload
      </Button>
      <Button color='secondary' size='lg' block onClick={reset}>
        Clear
      </Button>
    </>
  );
